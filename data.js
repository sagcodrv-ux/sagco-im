...
/* ── Status badge rules ────────────────────────────────────── */
var STATUS_RED  = ['OPEN','URGENT','CRITICAL','NOT DONE','NON-COMPLIANT','PENDING CEO SIGNATURE','FAILED','OVERDUE'];
var STATUS_ORG  = ['AT RISK','AMBER','MONITOR','DUE'];
var STATUS_AMB  = ['IN PROGRESS','PENDING','PLANNED'];
var STATUS_GRN  = ['ACTIVE','COMPLIANT','ON TRACK','COMPLETE','CLOSED','APPROVED','PASS','DONE'];
...
function renderTable(data, container) {
  if (!data || !data.headers || !data.rows) {
    container.innerHTML = '<div class="empty-state"><strong>No data</strong>Table is empty or not yet populated in Google Sheets.</div>';
    return;
  }
  var wide = data.headers.length > 8;
  var html = '<div class="tbl-wrap"><table class="tbl-stripe'+(wide?' tbl-compact':'')+'">';
  html += '<thead><tr>';
  data.headers.forEach(function(h) { html += '<th>'+h+'</th>'; });
  html += '</tr></thead><tbody>';
  if (!data.rows.length) {
    html += '<tr><td colspan="'+data.headers.length+'"><div class="empty-state"><strong>No records</strong>Add data in Google Sheets to populate this table.</div></td></tr>';
  } else {
...
function autoLoadPageData() {
  ...
  if (!key) { liveDiv.innerHTML = '<div class="empty-state"><strong>No data connection</strong>This page has no Google Sheets tab mapped.</div>'; return; }
  liveDiv.innerHTML = '<div style="padding:20px;text-align:center;color:var(--text-lt);font-size:11px">⏳ Loading live data…</div>';
  fetch(SHEETS_URL + '?tab=' + key + '&action=read')
    ...
    .catch(function() {
      liveDiv.innerHTML = '<div class="empty-state"><strong>Connection unavailable</strong>Could not reach Google Sheets. Check SHEETS_URL in data.js.</div>';
    });
}
