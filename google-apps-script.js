...
function saveDocumentRecord(params) {
  ...
  try {
    ...
    var cols = {
      ...
      deleted:        findCol(['Deleted']),

      deletedAt:      findCol(['Deleted At']),
...
function updateDocumentRev(docId, rev) {
  ...
  try {
    ...
    if (!sheet) {
      ...
      sheet = ss.getSheetByName(' Document Register');
...
/* ══════════════════════════════════════════════════════════════
   ...
   Tab: Approvals
   Headers (row 1): Approval ID | Doc ID | Rev | Approver |
     Approver Role | Status | Comments | Submitted By |
...
function getOrCreateApprovalsSheet() {
  ...
  if (!sh) {
    ...
    sh.getRange(1, 1, 1, hdr.length)
      .setBackground('#1a2340').setFontColor('#ffffff').setFontWeight('bold');
    sh.setFrozenRows(1);
  }
  return sh;
}
...
function saveApprovalRecord(p) {
  try {
    ...
    var hdr  = data[0];
    function col(name) {
      var i = hdr.indexOf(name); return i >= 0 ? i : -1;
    }
    /* Check if approval ID already exists */
    var existingRow = -1;
    ...
    if (existingRow > 0) {
      sh.getRange(existingRow, 1, 1, hdr.length).setValues([rowData]);
    } else {
      sh.appendRow(rowData);
    }
    /* Update Approval Status on Document Register */
...
function updateApprovalRecord(p) {
  try {
    ...
    var data = sh.getDataRange().getValues();
    var hdr  = data[0];
    function col(name) { var i = hdr.indexOf(name); return i; }
    ...
    /* Recalculate overall approval status for this doc+rev */
    if (found) {
      ...
      var docRecs = allRecords.slice(1).filter(function(row) {
        return String(row[c('Doc ID')]) === String(p.docId) &&
               String(row[c('Rev')])    === String(p.rev);
      });
      ...
      /* If all approved → set document Status to Active */
      if (allApprove) {
        setDocumentStatusActive(p.docId);
      }
      /* If rejected → set document Status back to Draft */
      if (hasReject) {
        setDocumentStatusDraft(p.docId);
      }
    }

    return ContentService.createTextOutput(
      JSON.stringify({ ok: true, found: found })
    ).setMimeType(ContentService.MimeType.JSON);
  } catch (e) {
    return ContentService.createTextOutput(
      JSON.stringify({ ok: false, error: e.message })
...
function updateDocApprovalStatus(docId, rev, approvalStatus) {
  try {
    ...
    if (!sh) return;
    var data = sh.getDataRange().getValues();
    var hdr  = data[2]; /* Row 3 = headers */
    var idCol = hdr.indexOf('Doc ID');
    var apCol = hdr.indexOf('Approval Status');
    if (idCol < 0 || apCol < 0) return;
    for (var r = 3; r < data.length; r++) {
      if (String(data[r][idCol]) === String(docId)) {
        sh.getRange(r + 1, apCol + 1).setValue(approvalStatus);
        break;
      }
    }
  } catch (e) { /* silent */ }
...
function setDocumentStatusActive(docId) {
  try {
    ...
    for (var r = 3; r < data.length; r++) {
      if (String(data[r][idCol]) === String(docId)) {
        sh.getRange(r + 1, stCol + 1).setValue('Active');
        break;
      }
    }
  } catch (e) { /* silent */ }
...
function setDocumentStatusDraft(docId) {
  try {
    ...
    for (var r = 3; r < data.length; r++) {
      if (String(data[r][idCol]) === String(docId)) {
        sh.getRange(r + 1, stCol + 1).setValue('Draft');
        break;
      }
    }
  } catch (e) { /* silent */ }
